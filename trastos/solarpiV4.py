...
def leer_comandos():
    with open("comandos.txt", "r+") as archivo:
        ...
        # aqui sehttps://www.riu.com/dcbaltest.critical cierra automaticamente el fichero.
    ...
    # Queremos que borre la primera linea del fichero comandos.txt
    # cojeremos el primer comando de la variable comandos
    # Por ultimo queremos que cierre el fichero comandos.txt

    # Finalmente devolvemos el comando y ademas se borrara el primer comando.
    return resultado
...
while True:
    ...
    if os.stat('comandos.txt').st_size == 0:
        print("    Vamos a descargar el archivo de internet")
        try:
            url = "http://s3.eu-north-1.amazonaws.com/ciudad94.com/comandos.txt"
            urllib.request.urlretrieve(url, 'comandos.txt')
        except socket.gaierror:
            print ("Ha habido un error al descargar el fichero!!")
        except urllib.error.URLError:
            print ("Ha habido un error al descargar el fichero")
        except:
            print ("Ha petatado algo y no sabemos el que")
        time.sleep(2)
    ...
    if el_comando.startswith("MANUAL"):
        ...
        # Si el comando recibido es autoatico on, movemos las pacas en modo automatic     >
        print("    Hemos recibido la orden de entrar en modo MANUAL")
    if el_comando.startswith("AUTOMATICO"):
        modo_automatico = True
        # Empezamos con la parte de mover las placas.
        print("    Hemos recibido la orden de entrar en modo AUTOMATICO")
    ...
    print("    El valor del sensor del EjeRotacion  es: ", vEjeRotacion )
    print("    El valor del sensor del EjeCabeceo   es: ", vEjeCabezeo )
    ...
    if not se_han_movido_las_placas:
        ...
        with open('logSolarPiV4.txt', 'a') as archivo_log:
            registro = f"{fechaleida}{frase}\n"
            archivo_log.write(registro)
    #Vamos a llamar a la osnda lamda con los valores que hay en las variables oEjeRotacion y oEjeCabezeo 
    if modo_automatico == True:
        modo_actual = "Automatico"
    else:
        modo_actual = "Manual"
        try:
            url = "http://5ynslybapapfth4mgupcrsjtj40mwvph.lambda-url.eu-north-1.on.aws/?operacion="+str(modo_actual)+"&oEjeRotacion="+str(vEjeRotacion)+"&oEjeCabeceo="+str(vEjeCabezeo)+"&Fecha=20240127_1500"
            urllib.request.urlretrieve(url, 'estado.txt')
        except:
            print ("Error al subir los comandos")
# Fin del programa.
